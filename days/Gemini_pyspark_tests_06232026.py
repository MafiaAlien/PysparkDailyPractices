...
class FinancialTuningPipeline:
    ...
    def calculate_rolling_spend(self, df):
        ...
        # TODO: Add time_seconds column using F.unix_timestamp() or cast("long")
        # TODO: Define W.partitionBy().orderBy().rangeBetween()
        # TODO: Apply window aggregate, round, select, and drop helpers
        window_spec_7days = W\
            .partitionBy(F.col("user_id"))\
            .orderBy(
                F.col("time_seconds").asc())\
        ...
        optimized_df = df\
            .select(
                'user_id',
                'order_time',
                F.unix_timestamp("order_time").alias('time_seconds'),
                'amount'
            ).withColumn(
                'rolling_7d_spend',
                F.sum(F.col('amount'))
