...
**Some of it is in Chinese.** `log/key_takeaways.md` and
`log/problem_log_and_roadmap.md` are working notes in Chinese prose with English
...
**Who wrote what.** The problem statements and the sealed references are
generated by an assistant from the roadmap, and Part 3 of every day file is an
independent AI solution pasted in unmodified. Part 1 — both implementations —
is mine, as are the `REVIEW_NOTES`, the `VERDICT`, and the Part 5 post-mortem.
The logs are written at the close of each day from what that
day actually produced — measured, not recalled.

**If you are evaluating this repo**, one day file end to end is the fastest
read — [`days/day24_device_hour.py`](days/day24_device_hour.py). Part 1 is my
solution in both APIs; Part 3 is the independent AI solution; `REVIEW_NOTES`
and `VERDICT` are what I predicted about it *before* running anything. Then
[`refs/day24_device_hour_ref.md`](refs/day24_device_hour_ref.md) for the trap
that day was built around and the measured plan notes, and
[`log/key_takeaways.md`](log/key_takeaways.md) for what accumulated across all
of them.
