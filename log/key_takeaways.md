...
- 窗口 row_number()=1 既保留所有列,又能确定性地挑行(靠 ORDER BY 的
  ...
  **限定(Day 24 实测修正)**:确定性**全部由 ORDER BY 承担**,`row_number`
  这个算子本身不提供任何东西。ORDER BY 的表达式若在分区内**恒定**——最典型的是
  **排了 partition key 自己**(`PARTITION BY device_id, reading_ts
  ORDER BY reading_ts`)——则全行平局,退化成 `dropDuplicates(keys)` 的任意挑选,
  而语法看上去仍像一个确定性的选择。**审计痕迹是假的。** 本条与 Day 8
  "orderBy(...).dropDuplicates 不是契约"是同一件事的两种伪装:那次排序在算子
  之外,这次排序在算子之内但不判别。
...
## 至少一次投递:入口去重 (Day 24)
- **at-least-once 重复的是信封,不是载荷。** `event_id` 由网关在**传输时刻**盖章、
  `ingest_ts` 由 collector 在**落库时刻**盖章——两者都是**一次传输**的属性,而
  重传是**另一次**传输。所以重发行带着全新的 `event_id` 和全新的 `ingest_ts`,
  整行去重、按 `event_id` 去重**都收不拢**。能存活重传的身份只有载荷携带的业务键
  (此处 `(device_id, reading_ts)`),而**选这个键是一个必须显式做出的建模决策**,
  不能默认成"看起来像主键的那一列"。
- **三种去重强度,从弱到强**:

  | 写法 | 收得拢什么 | 幸存者 |
  |---|---|---|
  | `dropDuplicates()` / `distinct()`(整行) | 只有**逐字节相同**的写入 | 无所谓 |
  | `distinct` on 载荷全列(含度量列) | 载荷相同的重传 | **冲突时两行都留** |
  | `dropDuplicates(业务键)` | 对的那批行 | **任意** |
  | `row_number() OVER (PARTITION BY 业务键 ORDER BY 判别键)` | 对的那批行 | **写死** |

  第二行是 Day 24 的 AI 解法所在的档位,比第三档还弱:一条**订正重发**(同键、
  `temp_c` 已修正)在它下面作为两行整个漏过去。
- **`ORDER BY ingest_ts DESC, event_id DESC` 钉的是三种情况**,在"副本逐字节相同"
  的数据上一种都不发生,所以它总是看起来冗余:
  (1) **订正重发**——坏读数被修复的正常方式就是按同键再发一条带新值的,"留哪条"
  从此是业务问题,`ingest_ts DESC` 就是 last-write-wins 的书面回答;
  (2) **重跑不幂等**——任意挑选意味着同一份输入、同一段代码、两次运行可以给出
  不同输出,下游数字变了而 git 里没有任何改动能解释它;
  (3) **契约变宽的那天**——多加一列(固件版本、校准偏移)之后副本立刻不再可互换,
  代码一个字没改就从"碰巧安全"静默滑到"随机挑选"。
  第二排序键的作用是把任意性**一路挤压到真正无法区分的行上**(Day 24 的 E06/E06
  连 `event_id` 都相同,此时的任意性不可观测)。
- **去重是一个位置决策,不只是键决策。** 它属于**入口**,在任何 join 与聚合之前。
  下游修补只能**逐指标**地修:`COUNT(DISTINCT reading_ts)` 修好了计数、`AVG`
  照错,于是契约每加一列就得重新推导一次补丁。
- **幂等聚合会掩护重复。** `MAX` / `MIN` 对重复行免疫;`COUNT` / `SUM` 不免疫;
  **`AVG` 只在被复制的值等于组均值时免疫**——Day 24 的数据恰好就是这个布置
  (两条幸存读数都是 22.0)。免疫的那批,恰恰**不成比例地是人类拿来抽查的那批**。
  这与 Day 23 的 `COUNT(DISTINCT)` 是同一族(见 Join 扇出一节):**先搞清哪些度量
  能活过你正在对行集做的那个变换。**
- **输出 grain 断言在结构上看不见入口重复**(Day 24 实测,Day 23 那条的升级):
  Day 23 记的是"扇出不改变输出形状,所以形状断言抓不到";Day 24 更强一档——最后的
  `groupBy` 让 grain **构造性唯一**,于是"grain 唯一"这个断言无论输入有没有去重
...
- **`left_anti` 与 `NOT EXISTS` 计划形状不同,SQL 侧更便宜**(Day 24 实测,
  AQE off,`shuffle.partitions=4`):同一个反连接,DSL 显式 `left_anti` 为 **4** 个
  Exchange、SQL `NOT EXISTS` 为 **3**。SQL 计划把 feed 按 `device_id` **单键**
  shuffle 一次,让 `Window` 直接跑在它上面——合法的原因是 `Window` 的
  `requiredChildDistribution` 是 `ClusteredDistribution(device_id, reading_ts)`,
  而 `HashPartitioning(device_id)` 作为其表达式集的**子集**满足它,一次 shuffle
  同时喂窗口和 join;DSL 计划则按全键排一次、再按 `device_id` 排一次。
  **分区键子集规则第六次确认**(Day 16/17/18/23),而且这次是在**两份独立编写的
  实现**(用户与 AI)上同时复现的——不是某一种写法的巧合。
  **Catalyst 为什么对 `NOT EXISTS` 改写取更小的键集、对显式 `left_anti` 不取,
  本日未确立**——是实测,不是推导。已排除的解释:调换 DSL 两个 join 的顺序无任何
  变化(仍 4 个 Exchange)。
- **列裁剪会让"未投影的配置列"这类发现在计划上完全不可见**(Day 24 实测):P3 要求
  只读配置表需要的列,用户 DSL 整表进 join、AI 显式 `.select`,但四条路线扫下来,
  `model#` / `ticket_id#` 出现在**非 Scan 节点**上的次数**全是 0**——ColumnPruning
  在 Scan 之上就剪掉了,写没写投影都一样。反过来,**`Scan ExistingRDD` 在所有路线
  上都吐全部五列**,包括显式投影的那条,所以在这个 harness 里**读 Scan 节点无法
  验证裁剪**(与 parquet 的 `ReadSchema` 不同)。推论:这类发现是**源码卫生**,
  不是字节代价,严重度按"两个分支是否一致"归到 production robustness,不要归到
  performance。与 Day 16"测计划前先确认被测算子还活着"同源:那条讲裁剪会删掉算子
  让你量到假计划,这条讲裁剪会**替你完成**你以为自己漏掉的事。
...
- **`ORDER BY` 是不是在 `PARTITION BY` 里面**(Day 24):看到
  `row_number() OVER (PARTITION BY k... ORDER BY x)` 形式的去重,**第一件事是检查
  `x` 是不是 `k` 的成员**,或在分区内是否恒定。是 -> 全行平局 -> 你写下的是
  `dropDuplicates(k)`,语法像确定性选择、保证不是,而且**没有任何一行代码看起来
  是错的**。追问:这个 ORDER BY 表达的业务规则是什么?"留最新到达的"用
  `ingest_ts`,"留最后传输的"用 `event_id`——**说不出理由就说明这个决策还没有被
  做出来**。这是 Day 22"多余的 tie-break 列默认在买确定性"的**反面**:那条说别把
  全序判成冗余,这条说**别把恒定序判成全序**。
- **去重键要把列摊开来并排比,不能只比算子**(Day 24):遇到对方的去重和你的不同,
  列出**两边的键列**再下判断。三条判据:(a) 键里含**度量列**
  (`DISTINCT device_id, reading_ts, temp_c`)就不是业务键——它只在载荷逐字节相同时
  折叠重传,一条订正重发会作为第二行整个漏过去;(b) 键取自**信封**(`event_id` /
  `ingest_ts` / 整行)在 at-least-once 的场合必然收不拢;(c) 键**必须**取自载荷,
